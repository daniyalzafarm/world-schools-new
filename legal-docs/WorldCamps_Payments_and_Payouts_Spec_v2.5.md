...
**Status:** Supersedes v2.4 in full. Locks the three standard-tier refund schedules (Flexible / Moderate / Strict) and a single 90/60/30 day-band lattice shared by the standard tiers and the Custom builder. All payment mechanics carried forward from v2.4 (reconciliation with Provider Terms v1.8, Customer Terms v1.5, Privacy Policy v1.5).
...
### 2.2 Standard-tier refund schedules (LOCKED) and the single day-band lattice

**Canonical day-band lattice.** All cancellation policies — the three standard tiers and every Custom policy — resolve against one lattice: **90+ / 60–89 / 30–59 / under 30** days before programme start, plus an implicit **after start = 0%** across all tiers. Boundaries are resolved per the day-counting convention in §3.6. This single lattice replaces the separate band sets used in earlier drafts.

**Standard-tier schedules.** Refund percentage applies to the **balance** (Booking price less deposit); the deposit is separately non-refundable after the 24-hour grace window (§3.2, CT §8.3).

| Tier | 90+ days | 60–89 days | 30–59 days | under 30 days | after start |
|---|---|---|---|---|---|
| **Flexible** | 100% | 100% | 100% | 0% | 0% |
| **Moderate** | 100% | 100% | 50% | 0% | 0% |
| **Strict** | 100% | 50% | 0% | 0% | 0% |

Equivalent plain-English form (used in the Help Centre / checkout copy):
- **Flexible** — full refund of the balance until 30 days before start; 0% thereafter.
- **Moderate** — full refund until 60 days before; 50% from 60 to 30 days; 0% within 30 days.
- **Strict** — full refund until 90 days before; 50% from 90 to 60 days; 0% within 60 days.

**Capture events derived per tier** (the engine schedules a capture at each boundary where the refund % drops, for the newly non-refundable portion of the balance):

| Tier | Internal shape | Capture event(s) on the balance |
|---|---|---|
| **Flexible** | Binary | 100% of balance at 30 days before start |
| **Moderate** | Two-Stage | 50% at 60 days; remaining 50% at 30 days |
| **Strict** | Two-Stage | 50% at 90 days; remaining 50% at 60 days |

**Deposit per tier.** **Flexible carries a zero deposit** (so "full refund until 30 days" is honest). Moderate and Strict use the Provider's configured deposit (camp-type default per §3.1, or per-Listing override per §3.2); the deposit is non-refundable after grace and sits on top of the balance schedule above.

**Custom** policies use the same lattice and the same derivation, subject to the rails in §3.4A (2–4 bands, monotonic non-increasing, 0–100%, 0% after start, deposit per §3.2, mandatory checkout one-liner, admin review before activation).
...
The Provider selects their camp type at onboarding. This pre-populates a **suggested standard tier** (per §2.2) and a **deposit default**. Both remain editable — the Provider may pick any standard tier, switch the deposit off, or configure a Custom policy. The capture events for a Provider on a standard tier are derived from the tier schedule in §2.2 (the Provider does not set capture dates directly); capture dates are set explicitly only when the Provider builds a Custom policy (§3.4A).

| Camp type | Deposit default | Suggested tier | Resulting capture events (from §2.2) |
|---|---|---|---|
| European day camp | 20% | Flexible | 100% of balance at 30 days before start |
| European residential | 20% | Moderate | 50% at 60 days; 50% at 30 days |
| US day camp | 15% | Flexible | 100% of balance at 30 days before start |
| US residential | 25% | Moderate (or Custom fixed-date) | 50% at 60 days; 50% at 30 days — or a Provider-set fixed calendar cutoff via Custom |
| Premium / international | 25% | Strict | 50% at 90 days; 50% at 60 days |

A Provider whose commitment dates do not match a standard tier (e.g. a day camp wanting a 14-day cutoff) configures a Custom policy (§3.4A).
...
> Note: the 90+/60–89/30–59/<30 lattice is shared by the Custom builder and the standard tiers (§2.2). Flexible / Moderate / Strict resolve to their locked preset schedules (§2.2); Custom lets the Provider set the percentage in each band directly. The same engine compiles all four.
...
- [ ] Internal: compile selected tier → refund schedule (§2.2 locked presets, or Custom band table) → derived capture events (engine)
...
Generated dynamically from the configured tier (§2.2). All strings use the named tiers; never "Binary/Two-Stage". Examples:

**Flexible (zero deposit):**
*"Full refund if you cancel 30 or more days before the programme starts. No refund within 30 days of the start date."*

**Moderate:**
*"Full refund of your balance if you cancel 60 or more days before the programme starts. 50% of your balance is refundable from 60 to 30 days before start; no refund within 30 days. Deposit non-refundable 24 hours after your booking request."*

**Strict:**
*"Full refund of your balance if you cancel 90 or more days before the programme starts. 50% of your balance is refundable from 90 to 60 days before start; no refund within 60 days. Deposit non-refundable 24 hours after your booking request."*

**Custom (fixed calendar date, 1 March):**
...
For no-deposit / Flexible, omit the deposit line. Where the 7-day exclusion applies, replace the deposit line with: *"Because this programme starts soon, the 24-hour free-cancellation window does not apply."*
...
| Item | Type | Status |
|---|---|---|
| Standard-tier refund schedules + day-band lattice | Product/Legal | **LOCKED 2026-06-12 (§2.2)** |
...
| **v2.5** | **2026-06-12** | **Standard-tier schedules LOCKED. (1) New §2.2: the three standard tiers locked — Flexible 100/100/100/0, Moderate 100/100/50/0, Strict 100/50/0/0 — on a single canonical 90+/60–89/30–59/<30 day-band lattice now shared by the standard tiers and the Custom builder (resolving the earlier two-lattice divergence). Per-tier capture events spelled out (Flexible: 100% at 30d; Moderate: 50% at 60d + 50% at 30d; Strict: 50% at 90d + 50% at 60d). Flexible = zero deposit; Moderate/Strict use the configured deposit. (2) §3.1 recast: camp type now pre-populates a suggested standard tier + deposit default, with capture events derived from the tier (not set independently); independent capture dates apply to Custom only. (3) §3.4A note, §15 customer-facing strings, and §16 open-items updated to reflect the lock. No change to payment mechanics or the contractual documents (the schedules live in the spec, Help Centre, and checkout — not in CT/PT, by design). All four named tiers retained including Strict (per Product–Legal Audit T-3 / P1-24 — add the Strict UI template rather than drop the tier).** |
