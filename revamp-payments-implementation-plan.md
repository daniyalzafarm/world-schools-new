...
- **Named tier → refund schedule:** `resolveTiers()` already compiles named tiers. **Add `STRICT_POLICY_TIERS`** to `packages/wc-types/src/lib/cancellation-policy.types.ts` + a `strict` branch (only `FLEXIBLE`/`MODERATE` exist today). `CaptureMode` is derived for observability only (1 drop → binary, 2 → two_stage, >2/custom → custom); never shown to users.
...
1. Flexible/Moderate/Strict preset bands & percentages — product/commercial lock pending (Alex). Build engine + Custom builder now; seed presets via config/migration (not enums) when locked; Flexible bound to zero deposit.
