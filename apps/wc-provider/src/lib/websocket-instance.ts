...
globalWsService.on('connection:auth_expiring', async () => {
  try {
    ...
    const result = await apiClient.post<{ accessToken: string }>('provider/auth/refresh', {
    ...
    if (newToken) {
      ...
      console.log('[WC Provider] WebSocket session token refreshed silently')
    }
  } catch (err) {
    console.warn('[WC Provider] Failed to refresh WebSocket token:', err)
  }
})

// If session has expired (e.g. refresh token also expired), redirect to sign-in
globalWsService.on('connection:auth_expired', () => {
  console.warn('[WC Provider] WebSocket session expired, redirecting to sign-in')
