...
import { type ImportCampRowError, providersService } from '@/services/providers.services'
...
export default function ImportCampsPage() {
  ...
  const providerName = detail?.legalCompanyName ?? 'Provider'
  ...
  const handleImport = async () => {
    ...
    setPhase('uploading')
    try {
      const data = await providersService.importCamps(providerId, selectedFile)
      setResult(data)
      setPhase('results')
    } catch (err: any) {
      setPhase('select')
  ...
  const downloadTemplate = () => {
    const lines = CAMP_IMPORT_COLUMNS.map(c => {
    ...
    a.href = url
    a.download = 'camp-import-template.csv'
    a.click()
    URL.revokeObjectURL(url)
  }

  const requiredColumns = CAMP_IMPORT_COLUMNS.filter(c => c.required)
  const optionalColumns = CAMP_IMPORT_COLUMNS.filter(c => !c.required)
  ...
  return (
    <PageSlot>
      <div className="space-y-6">
        ...
        {/* Page header */}
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Import Camps</h1>
          <p className="mt-1 text-sm text-default-500">
            {phase === 'results'
              ? 'Import complete — review the results below.'
              : `Upload a CSV to create multiple draft camps for ${providerName}. Sessions can be added later via the provider portal.`}
          </p>
        </div>
                ...
                <DocumentDropzone
                  ...
                  description={
                    selectedFile
                      ...
                      : `.csv only · max ${MAX_FILE_SIZE_MB} MB · max 500 camps`
                  }
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold">
                      {result.imported > 0
                        ? `${result.imported} camp${result.imported !== 1 ? 's' : ''} imported successfully`
                        : 'No camps were imported'}
                    </p>
                    {result.failed > 0 && (
                      <p className="mt-0.5 text-sm text-default-600">
                        {result.failed} camp{result.failed !== 1 ? 's' : ''} failed — see the error
                        details below.
                      </p>
                    )}
                  </div>
