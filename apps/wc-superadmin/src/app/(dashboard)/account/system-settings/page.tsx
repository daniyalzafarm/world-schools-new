...
export default function SystemSettingsPage() {
  const [defaultCommission, setDefaultCommission] = useState<string>('')
  ...
  const loadSettings = async () => {
    ...
    if (response.success && response.data) {
      setDefaultCommission(String(response.data.defaultCommission))
  ...
  const handleSave = async () => {
    const parsed = parseFloat(defaultCommission)
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
            <div className="border-b border-slate-200 px-6 py-4 dark:border-slate-700">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Default commission percentage charged on each booking. This rate is snapshotted onto
