...
export function useUnreadNotificationsCount(): number {
  ...
  const { playSound, showNotification, requestPermission } = useNotifications({
    storageKey: 'wc_superadmin_notification_preferences',
  })

  const fetchCount = useCallback(async () => {
    if (!user?.id) return
    const result = await apiClient.get<{ count: number }>('/superadmin/notifications/unread-count')
    if (result.success) {
  ...
  useEffect(() => {
    ...
    requestPermission().catch(error => {
      console.error(
        '[useUnreadNotificationsCount] Failed to request notification permission:',
        error
      )
    })
