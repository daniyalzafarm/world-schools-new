...
/**
 * Provider Conversations Controller
 *
 * App-specific wrapper for conversation endpoints used by wc-provider app.
 * All endpoints are prefixed with /provider/messaging/conversations
 *
 * This controller delegates to the shared ConversationsService but ensures:
 * - JWT strategy uses wc_provider_access_token cookie (based on /provider/* path)
 * - Proper token isolation between apps
 * - Consistent API structure with other provider endpoints
 */
