...
// Parent inbox tabs. Archived shows archived threads; Favorites shows starred
// non-archived threads; All shows the rest of the non-archived threads.
type MessageFilter = 'all' | 'favorites' | 'archived'
...
const EMPTY_STATE_COPY: Record<MessageFilter, { title: string; subtitle: string }> = {
  ...
  favorites: { title: 'No favorites', subtitle: 'Conversations you star will appear here' },
  archived: {
...
export const MessagesSidebar: React.FC<MessagesSidebarProps> = ({
  ...
  // The Archived tab shows archived threads; the others show non-archived
  // threads (Favorites = starred, All = everything else).
  const filteredConversations = useMemo(() => {
    ...
    switch (activeFilter) {
      case 'archived':
        ...
        break
      case 'favorites':
        filtered = userConversations.filter(conv => !conv.archived && conv.starred)
        break
      case 'all':
      default:
        filtered = userConversations.filter(conv => !conv.archived)
  ...
  const favoritesUnread = userConversations.filter(
    conv => conv.starred && !conv.archived && conv.unread
  ).length
  ...
  const handleConversationPress = useCallback((conversation: Conversation) => {
    ...
    // (optimistic clear + persist), so we don't duplicate it here. Selection is
    // event-based with no URL change — the layout switches list↔chat on mobile
    // from the active conversation (state-based, matching wc-provider).
                ...
                <div className="flex max-w-md gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {(
                    [
                      ...
                      { key: 'favorites', label: 'Favorites', count: favoritesUnread },
