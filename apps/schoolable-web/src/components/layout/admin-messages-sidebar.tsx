...
export const AdminMessagesSidebar: React.FC<AdminMessagesSidebarProps> = ({
  sidebarOpen,
  setSidebarOpen,
}) => {
  ...
  const filteredConversations = useMemo(() => {
    let filtered = adminConversations.filter(conv =>
  ...
  }, [adminConversations, activeFilter, searchQuery, isArchivedPage])

  // Count for different filter types and archived conversations
  const allCount = adminConversations.filter(conv => !conv.archived).length
  const favoritesCount = adminConversations.filter(conv => conv.starred && !conv.archived).length
  const unreadCount = adminConversations.filter(
    conv => !conv.archived && (conv.unread || (conv.unreadCount && conv.unreadCount > 0))
  ).length
  const archivedCount = adminConversations.filter(conv => conv.archived).length

  // Handle conversation press
  const handleConversationPress = useCallback(
    (conversation: Conversation) => {
      // Mark conversation as read
      markAsRead(conversation.id, true)
            ...
            <ScrollShadow className="flex-1" hideScrollBar>
              ...
              {!isArchivedPage &&
                ...
                !(searchQuery.trim() !== '' && filteredConversations.length === 0) && (
                  <>
                    <div className="">
                      <ArchivedChatsButton archivedCount={archivedCount} isAdmin={true} />
                    </div>
                ...
                filteredConversations.map(conversation => (
                  <React.Fragment key={conversation.id}>
                    <ConversationItem
                      ...
                      onArchive={id => toggleArchive([id], true)}
                      onDelete={id => deleteConversations([id], true)}
                      onMute={id => toggleMute([id], true)}
                      onMarkAsUnread={id => markAsUnread([id], true)}
                      onBlock={id => blockConversations([id], true)}
                      onToggleFavorite={id => toggleFavorite([id], true)}
                    />
                    {/* Divider behavior mirrors user sidebar: no extra margins */}
                    {/* Keeping consistent spacing with user version */}
                  </React.Fragment>
                ))
              )}
            </ScrollShadow>
