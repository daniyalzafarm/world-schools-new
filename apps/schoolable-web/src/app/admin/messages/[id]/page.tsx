...
export default function AdminConversationPage() {
  ...
  const conversation = adminConversations.find(conv => conv.id === conversationId)

  // If conversation not found, redirect to messages page
  useEffect(() => {
    if (adminConversations.length > 0 && !conversation) {
      router.push('/admin/messages')
    }
  }, [conversation, adminConversations.length, router])
  ...
  if (!conversation && adminConversations.length > 0) {
    return (
